...
<#
.SYNOPSIS
    Deploys the HealthLogger application to Azure App Service.
...
.PARAMETER OpenBrowser
    Open the deployed application in the default browser after a successful deployment. Disabled by default.

.EXAMPLE
    .\deploy-to-azure.ps1 -AppName "healthlogger" -ResourceGroup "rg-app-services"

.EXAMPLE
    .\deploy-to-azure.ps1 -AppName "healthlogger-dev" -ResourceGroup "rg-dev" -BuildConfiguration Debug

.EXAMPLE
    .\deploy-to-azure.ps1 -AppName "healthlogger" -ResourceGroup "rg-app-services" -WhatIf

.EXAMPLE
    .\deploy-to-azure.ps1 -OpenBrowser
#>

param(
    [Parameter(Mandatory = $false, HelpMessage = "Azure App Service name")]
    [string]$AppName = "healthlogger",
...
╔══════════════════════════════════════════════════════════╗
║                                                          ║
║     HealthLogger - Azure App Service Deployment          ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝
...
Write-Info "Open Browser: $OpenBrowser"
Write-Host ""
...
if (-not $SkipBuild) {
    ...
    Pop-Location
    $publishPath = "HealthLogger/bin/publish"
} else {
...
Write-Host "  • Production uses SQL Server (connection string in App Service config)"
...
# Open the browser only when explicitly requested.
if ($OpenBrowser) {
